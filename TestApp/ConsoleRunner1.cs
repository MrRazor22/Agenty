...
//                .WithTools<MathTools>()
//            //.W.WithFlow(new ToolCallingStep());ithExecutor(new RagToolCallingExecutor(maxRounds: 50));
//            //.WithExecutor(new PlanningExecutor(maxRounds: 50)); 


//            var kb = new RagRetriever(embeddings, new FileVectorStore(Path.Combine(solutionRoot, "Agenty", "Test")));
